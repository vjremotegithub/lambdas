'''
Function Name:  generateIamUsersReport_v2.py
...
def lambda_handler(context,event):
    ...
    auditTable = dynamodb.Table('IAM_USERS_AUDIT')
    response = table.scan()
...
def generateReport(roleName,awsAccountName, awsAccountId, auditTable):
    ...
    for row in credReport:
        if row['password_enabled'] == "false":
            ...
            sleep(0.005)
            auditDate=datetime.today().strftime('%Y-%m-%d %H:%M')
            try:
                    ...
                    'RowId':str(dateInMilliSecondsNow)
                    } 
                    )
            except Exception, e:
                print (e.message)
